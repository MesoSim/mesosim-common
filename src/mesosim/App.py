...
class Team:
    ...
    def write_status(self):
        ...
        if self.active_hazard is None:
            ...
            self.status["hazard_exp_time"] = self.active_hazard.expiry_time.strftime(
                db_time_fmt
            )
        ...
        self.cur.execute(
            (
                "INSERT INTO team_history (timestamp, latitude, longitude, "
    ...
    def output_status_dict(self):
        ...
        direction_lock = False or (
            self.active_hazard is not None and self.active_hazard.direction_lock
        )
        speed_lock = False or (
            self.active_hazard is not None and self.active_hazard.speed_lock
        )
    ...
    def alter_status(self, status):
        ...
        if self.is_adjustment:
            ...
            if self.action_type == "set":
                ...
                status[self.action_field] = float(status[self.action_field]) + float(
                    self.action_amount
                )
        return status
