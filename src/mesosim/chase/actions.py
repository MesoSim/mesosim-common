...
class Hazard(Action):
    ...
    def __init__(
        ...
        self.alter_status = alter_status  # function(team)
        self.probability = probability  # function(team)
...
def create_hazard_registry(config):
    ...
    # Speeding #
    ############
    def speeding_alter_status(team):
    ...
    # Dirt Road #
    #############
    def dirt_road_alter_status(team):
    ...
    # Stuck in mud #
    ################
    def stuck_in_mud_alter_status(team):
        team.speed = 0.0
        team.status_color = "red"
        team.status_text = "Stuck in mud!"

    def stuck_in_mud_prob(team):
    ...
    # Chaser convergence #
    ######################
    cc_speed_limit = np.random.choice(np.arange(15, 50, 5))

    def cc_alter_status(team):
        team.speed = cc_speed_limit
        team.status_color = "yellow"
    ...
    # Flat tire #
    #############
    pay_for_flat = np.random.random() < config.hazard_config("pay_for_flat_prob")

    def flat_tire_alter_status(team):
        team.speed = 0.0
        team.status_color = "red"
...
def shuffle_new_hazard(team, seconds, hazards):
    ...
    hazard_probs = [seconds / 60 * hazard.probability(team) for hazard in hazard_list]
